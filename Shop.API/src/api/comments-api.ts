...
commentsRouter.get('/:id',
    ...
    async (req: Request<{ id: string }>, res: Response) => {
        try {
            ...
            const [targetComment] = await connection.query<ICommentEntity[]>(
                'SELECT * FROM comments WHERE comment_id = ?',
                [req.params.id] // Здесь по правилам синтаксиса запроса передаваемые значения должны быть в массиве
            );
            ...
            res.status(200);
            res.setHeader('Content-Type', 'application/json');
            res.send(mapCommentsEntity(targetComment));
        } catch (e) {
            console.debug(e.message);
            res.status(500);
            res.send('Something went wrong');
